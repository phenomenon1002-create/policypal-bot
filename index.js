...
async function replyMsg(token, messages) {
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + CHANNEL_TOKEN, 'Content-Length': Buffer.byteLength(body) }
...
function menuBtn(emoji, label, text) {
  return {
    ...
    contents: [
      ...
      { type: 'text', text: label, size: 'sm', align: 'center', weight: 'bold', color: '#1e293b', margin: 'xs' }
    ]
  };
}
...
async function handleMsg(event) {
  ...
  if (!user) {
    if (/^09\d{8}$/.test(txt)) {
      ...
      const policies = await sb(`policies?client_id=eq.${c.id}&select=*`);
    ...
    return replyMsg(token, {
      ...
      contents: {
        ...
        header: { type: 'box', layout: 'vertical', backgroundColor: '#0f1f3d', paddingAll: '20px',
          contents: [
            { type: 'text', text: '🛡️ PolicyPal', color: '#ffffff', weight: 'bold', size: 'xxl', align: 'center' },
            { type: 'text', text: '保單夥伴 · Insurance Partner', color: '#94a3b8', size: 'sm', align: 'center', margin: 'sm' }
          ]
        },
        body: { type: 'box', layout: 'vertical', paddingAll: '20px', spacing: 'md',
          contents: [
            { type: 'text', text: '請輸入您的手機號碼', size: 'md', weight: 'bold', align: 'center', color: '#1e293b' },
            { type: 'text', text: '格式：0912345678', size: 'sm', color: '#94a3b8', align: 'center' },
            { type: 'box', layout: 'vertical', backgroundColor: '#f8fafc', cornerRadius: '10px', paddingAll: '12px', margin: 'md',
              contents: [{ type: 'text', text: '📱 輸入手機號碼即可查看您的保障內容', size: 'sm', color: '#475569', wrap: true, align: 'center' }]
            }
          ]
        }
      }
  ...
  if (txt.includes('開刀') || txt.includes('手術')) {
    ...
    if (!dl.length && !sl.length) return replyMsg(token, { type: 'text', text: '您目前沒有手術保障\n建議洽業務員評估！', quickReply: quickReply([{label:'回主選單',text:'選單'}]) });
  ...
  if (txt.includes('所有保單')||txt.includes('保單清單')) {
    if (!policies.length) return replyMsg(token, { type: 'text', text: '您目前尚無保單資料', quickReply: quickReply([{label:'回主選單',text:'選單'}]) });
    ...
    return replyMsg(token, [flex, { type: 'text', text: `您共有 ${policies.length} 張有效保單 🛡️`,
  ...
  if (txt.includes('保單健診')||txt.includes('健診')) {
    ...
    const hasCancer = policies.some(p => p.type==='癌症險'||p.type==='重大疾病險');
    const hasMaternity = policies.some(p => p.maternity_benefit > 0);
    let score = 0;
    if (totalDaily >= 3000) score += 30; else if (totalDaily >= 1500) score += 20; else if (totalDaily > 0) score += 10;
    if (totalLump >= 100) score += 30; else if (totalLump >= 50) score += 20; else if (totalLump > 0) score += 10;
    if (hasCancer) score += 25;
    if (hasMaternity) score += 15;
    const grade = score >= 80 ? '優秀 🏆' : score >= 60 ? '良好 👍' : score >= 40 ? '普通 ⚠️' : '需加強 🔴';
    const flex = {
      ...
      contents: {
        ...
        body: { type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '14px',
          contents: [
            { type: 'box', layout: 'vertical', backgroundColor: '#f0f9ff', cornerRadius: '12px', paddingAll: '16px', alignItems: 'center',
              contents: [
            ...
            flexRow('🏥 住院日額', totalDaily ? `$${totalDaily.toLocaleString()}/天` : '無', totalDaily >= 2000 ? '#10b981' : '#ef4444'),
            { type: 'separator', margin: 'sm' },
            flexRow('💊 重大疾病', hasCancer ? `${totalLump}萬` : '未投保', hasCancer ? '#10b981' : '#ef4444'),
            { type: 'separator', margin: 'sm' },
            flexRow('👶 生育給付', hasMaternity ? '有' : '未投保', hasMaternity ? '#10b981' : '#94a3b8'),
            { type: 'separator', margin: 'sm' },
            { type: 'box', layout: 'vertical', backgroundColor: score < 60 ? '#fee2e2' : '#d1fae5', cornerRadius: '8px', paddingAll: '12px', margin: 'sm',
              contents: [
